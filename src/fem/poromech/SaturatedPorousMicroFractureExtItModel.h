...
/** @file SaturatedPorousMicroFractureExtItModel.h
...
/** @brief 
 *  The SaturatedPorousMicroFractureExtItModel class implements a saturated
 *  porous media FE model with unified micromorphic phase-field fracture.
 *  Convexification via extrapolation is carried out followed by iterative
 *  correction. <a href="https://link.springer.com/article/10.1007/s00466-023-02380-1" target="_blank">Link to Article</a>
 * 
 *  Model options include randomly distributed porosity, different
 *  permeability models. The model also updates the time in globdat,
 *  so that Paraview Module can print time stamps to a pvd file.
 * 
 */

class SaturatedPorousMicroFractureExtItModel : public Model
{
 public:

  typedef SaturatedPorousMicroFractureExtItModel     Self;
  typedef Model                                      Super;
  ...
  static vector<String>     permeabilityModels;
                            ...
                            SaturatedPorousMicroFractureExtItModel
 ...
 protected:
  ...
  virtual                  ~SaturatedPorousMicroFractureExtItModel  ();
