...

{
  ...
  // Material of gradient enhanced damage model MUST be a Damage material
    ...
    ( MatrixBuilder&          mbuilder )
{
  ...
  double      rho = 0.0; // material_->giveRho ( );
...
void GradientDamageModel::setConstraints_ ( Constraints& cons )
{
  ...
  // the two independent nodes. Note that the DOF indices are local
  // with respect to an element. They will be translated to global DOF
  // indices below.
...
void GradientDamageModel::getStress_
...

{
  ...
  for ( int ie = 0; ie < ielemCount; ie++ )
  {
    ...
    for ( int ip = 0; ip < ipCount; ip++, igpoint++ )
    {
      ...
      material_->update ( stressIp, stiff, strain_(ALL,ipoint), ipoint );
...
void GradientDamageModel::getDamage_
